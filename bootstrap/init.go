...
func init() {
	...
	if variable.ConfigGormv2Yml.GetInt("Gormv2.Sqlserver.IsInitGlobalGormSqlserver") == 1 {
		if dbSqlserver, err := gorm_v2.GetOneSqlserverClient(); err != nil {
			...
			log.Println("数据库类型->", variable.ConfigGormv2Yml.GetString("Gormv2.UseDbType"))
		}
	}
	...
	// 11.订单处理服务启动
	if variable.ConfigYml.GetInt("Order.Start") == 1 {
		...
		case "yhcv2":
			...
			go (&order.Yhcv2{}).Run(
				...
				variable.ConfigYml.Get("Order"))
		default:
			log.Println("未识别 SellVersion 类型!")
			return
		}
		log.Println("云平台订单处理:启动")
		log.Println("Redis listkey:", variable.ConfigYml.GetString("App.Name"))
