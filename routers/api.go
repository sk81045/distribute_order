...
import (
	...
	"goskeleton/app/http/controller/api/dealings"
...
func InitApiRouter() *gin.Engine {
	...
	vApi := router.Group("/api/v1/")
	{
		...
		home := vApi.Group("home/")
		{
			...
			home.GET("homelist", validatorFactory.Create(consts.ValidatorPrefix+"List"))
		}

		// 模拟一个首页路由
		dealSort := vApi.Group("dealings/")
		{

			switch variable.ConfigYml.GetString("Order.SellVersion") {
			case "smv1":
				dealSort.GET("infos", validatorFactory.Create(consts.ValidatorPrefix+"UserInfo"))
				dealSort.GET("info", (&dealings.Smv1{}).UserInfo)
				dealSort.GET("list", validatorFactory.Create(consts.ValidatorPrefix+"List"))
				dealSort.GET("setlist", (&dealings.Smv1{}).SetList)
			case "yhcv1":

			case "yhcv2":
				dealSort.GET("info", (&dealings.Yhcv2{}).UserInfo)
				dealSort.GET("list", validatorFactory.Create(consts.ValidatorPrefix+"List"))
				dealSort.GET("setlist", (&dealings.Yhcv2{}).SetList)
			}
		}
	}
	return router
